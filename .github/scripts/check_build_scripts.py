...
* Every file beneath ``--repository`` whose basename case-folds to ``build.rs``
...
are the only excluded trees. Traversable directory symlinks fail closed because
otherwise a path alias could redirect traversal or conceal repository source
inputs. Broken or cyclic symlinks expose no files at check time; symlinks whose
own names are policy inputs are still discovered and rejected.
"""
