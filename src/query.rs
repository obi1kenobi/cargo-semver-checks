...
mod tests {
    ...
    fn test_data_is_fresh() -> anyhow::Result<()> {
        ...
        for id in SemverQuery::all_queries().into_keys() {
            for version in ["new", "old"] {
                let test_crate_path = format!("./test_crates/{id}/{version}");
                // skip lints without test crates
                if !std::fs::exists(&test_crate_path).unwrap_or_default() {
                    continue;
                }
                ...
                let localdata_path = format!("./localdata/test_data/{id}/{version}");
                    ...
                    (test_crate_times.last(), localdata_times.first())
                {
                    if test_max > local_min {
                        panic!("Files in the '{test_crate_path}' directory are newer than the local data generated by \n\
                            scripts/regenerate_test_rustdocs.sh in '{localdata_path}'.\n\n\
